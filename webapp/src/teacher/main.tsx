    ...
    const syncing = async () => {
      try {
        ...
        const syncStudentImgs = async (studentIdsJson: string | null) => {
          ...
          //check if respose has delete image commond
          if (data.hasOwnProperty('Delete_image')) {
            const storeImgs = localStorage.getItem('Students_Dataset');
            const storeIds = localStorage.getItem('Student_Ids');
            const prevArrIds: number[] = storeIds ? JSON.parse(storeIds) : [];
            const prevObjImgs: { label: number, descriptor: number[] }[] = storeImgs ? JSON.parse(storeImgs) : [{}];
            if (prevArrIds.length !== 0 && prevObjImgs.length !== 0) {
              const delete_id: number[] = data.Delete_image;

              const updatedImgs = prevObjImgs.filter((row, index) => !delete_id.includes(row.label))
              const updatedIds = prevArrIds.filter((id, index) => !delete_id.includes(id))
              localStorage.setItem('Students_Dataset', JSON.stringify(updatedImgs));
              localStorage.setItem('Student_Ids', JSON.stringify(updatedIds));

            }

          }

          //check if response has add image command
